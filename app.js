...
const state = { tab: 'home', circle: null, circleView: 'feed', circleCategory: '全部', moreOpen: false, security: false, liveSection: '互动', toast: null, joined: new Set(), reminders: new Set(), booked: false, notifications: new Set(['圈主更新', '评论互动', '任务提醒']), liveCart: new Set(), couponClaimed: false };

const circles = [
  { id: 'research', icon: '研', tone: '', title: '每日投研 · 盘面拆解', desc: '盘前策略 / 盘中观点 / 收盘复盘', members: '12,860', active: '今日 38 条动态', post: '王老师：午后关注新能源与券商板块的量能变化。' },
  { id: 'class', icon: '学', tone: 'gold', title: '趋势技能提升营', desc: '课程学员专属陪跑圈', members: '3,428', active: '今日 126 人学习', post: '第 6 讲回放已更新，完成课后练习可解锁案例。' },
  { id: 'vip', icon: 'VIP', tone: 'red', title: '高阶会员服务圈', desc: '策略 / 工具 / 专属答疑', members: '860', active: '仅会员可见', post: '本周策略会议将于周五 20:00 开始，点击预约。' },
];
...
function home() {
  return `<div class="screen">${shell('金脉', '自主品牌 · 私域服务中枢', { action: `<button class="icon-btn" data-tab="messages" aria-label="查看消息">♢<b class="badge">3</b></button>` })}
    ...
    <section class="section"><div class="section-head"><h3>私域服务</h3><button class="more" data-tab="circles">查看全部 ›</button></div><div class="quick-grid"><button class="quick" data-tab="circles"><i>◉</i>我的圈子</button><button class="quick" data-action="live"><i>◷</i>直播预约</button><button class="quick" data-tab="messages"><i>♢</i>消息中心</button><button class="quick" data-tab="profile"><i>★</i>会员权益</button></div><button class="course-promo" data-tab="learning"><div><b>学习中心</b><span>课程、训练营与学习进度</span></div><strong>继续学习　›</strong></button></section>
...
function render() {
  if (state.security) app.innerHTML = security();
...
document.addEventListener('click', event => {
  ...
  if (tab) { state.tab = tab.dataset.tab; state.security = false; state.circle = null; state.circleView = 'feed'; state.moreOpen = false; state.courseId = null; state.lesson = false; render(); return; }
